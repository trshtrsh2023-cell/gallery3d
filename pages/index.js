...
export default function Gallery(){
  ...
  const visitorsRef = useRef([]); // updated by poll, read by render loop
  ...
  const broadcast = useCallback(async(name)=>{
    ...
    try{ await fetch('/api/visitors',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({visitor_id:getVid(),name,x:s.pos.x,z:s.pos.z,yaw:s.yaw})}); }catch(e){}
  },[]);
  ...
  const poll = useCallback(async()=>{
    try{
      ...
      const filtered=(Array.isArray(d)?d:[]).filter(v=>v.visitor_id!==getVid());
      visitorsRef.current=filtered;
      setVisitorsUI(filtered);
    }catch(e){}
  },[]);
  ...
  useEffect(()=>{
    ...
    const p=setInterval(()=>broadcast(visitorName),2500);
    const q=setInterval(poll,2500);
    poll();
    return()=>{
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      const loop=()=>{
        ...
        // ── VISITOR AVATARS (managed in render loop, fresh data every frame) ──
        const currentVisitors = visitorsRef.current;
        const activeIds = new Set(currentVisitors.map(v=>v.visitor_id));
        ...
        // Add/update avatars
        currentVisitors.forEach((v,i)=>{
          if(!avatarMeshes[v.visitor_id]){
            avatarMeshes[v.visitor_id]=createAvatar(T,v.name,i);
            scene.add(avatarMeshes[v.visitor_id]);
          }
          const av=avatarMeshes[v.visitor_id];
          av.position.set(v.x||0, 0, v.z||21);
          av.rotation.y=(v.yaw||0)+Math.PI;
          ...
          // Idle walk animation (leg bob)
          if(frameN%2===0){
            av.children.forEach((ch,ci)=>{if(ci===0)ch.position.y=.65+Math.sin(now*.0024)*0.014;});
          }
        });
