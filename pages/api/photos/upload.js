...
export default async function handler(req, res) {
  ...
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      const { error: upErr } = await sb.storage.from('gallery-images').upload(fileName, buffer, { contentType: file.mimetype });
      if (upErr) return res.status(500).json({ error: upErr.message });
      const { data: urlData } = sb.storage.from('gallery-images').getPublicUrl(fileName);
      const { data, error: dbErr } = await sb.from('photos')
